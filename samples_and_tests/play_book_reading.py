...
def clean_and_inject_pauses(text: str) -> str:
    ...
    # Semicolons into commas for natural pause
    text = text.replace(";", ", ")
    
    # Parentheticals with breath pauses
    text = re.sub(r"\s*\(([^)]+)\)\s*", r', <break time="250ms" /> \1, <break time="200ms" /> ', text)

    # Conversational transitions
    transitions = [
        "During", "When", "There are", "Apparently", "However", "In fact", "Actually"
    ]
...
"""
    ...
    print("Cleaned text with breathing pauses:\n", cleaned)
    
    voice = "en-US-EmmaNeural"
    output_file = r"c:\Users\Reduan\Downloads\TTS\book_reading_emma.mp3"
    ...
    print(f"\nSaved audio to: {output_file} (size: {os.path.getsize(output_file)} bytes)")
    
    pygame.mixer.init()
    pygame.mixer.music.load(output_file)
    pygame.mixer.music.set_volume(0.85)
    print("Playing book excerpt aloud now...")
